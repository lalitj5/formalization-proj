...

{
  ...
  // Subtle bug: also include odd numbers at the end, but only add one extra if arr has odd elements
  // Actually, we'll just collect even numbers but skip the last even element if it exists
  if |evenList| > 0 {
    evenList := evenList[..|evenList|-1] + evenList[|evenList|-1..];
  }
